...
const LessonPlayer = ({ lesson, onBack }: LessonPlayerProps) => {
  ...
  const lessonNumber = title.match(/\d{3}/)?.[0] || "";

  // הגדרת בסיס הכתובת המעודכן
  const MEDIA_BASE_URL = "https://ateretmordechaifiles.com/mediaFiles";
  
  // פונקציה לבניית כתובת תקינה כולל קידוד רווחים
// פונקציה לבניית כתובת תקינה ונקייה עבור נטפרי
  const buildSafeUrl = (dir: string, fileName: string) => {
  if (!dir || !fileName) return null;

  // 1. ניקוי ה-dir: מסירים סלאשים מההתחלה ומהסוף כדי שלא יתנגשו עם הבסיס
  const cleanDir = dir.replace(/^\/+|\/+$/g, '');
  
  // 2. פירוק הנתיב לחלקים וקידוד כל חלק בנפרד (כדי לטפל ברווחים ומקפים)
  const encodedPath = cleanDir
    .split('/')
  ...
  // 4. חיבור הכל לכתובת אחת תקנית
  return `${MEDIA_BASE_URL}/${encodedPath}/${encodedFileName}`;
};
  const audioUrl = buildSafeUrl(AudioDir, AudioFileName);
  const pdfUrl = buildSafeUrl(AudioDir, PdfFileName);
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#fff', py: { xs: 2, md: 4 }, px: 2, direction: 'rtl' }}>
      <Container maxWidth="md">
        <Box sx={{ 
          ...
          {/* נגן שמע - הוספת key לרענון הנגן כשמחליפים שיעור */}
          {audioUrl && (
            <Box sx={{ mb: 4, p: 2, bgcolor: 'rgba(156, 102, 68, 0.1)', borderRadius: '15px', border: `1px dashed ${COLORS.BROWN_DARK}` }}>
                ...
                <audio controls key={audioUrl} style={{ width: '100%', height: '40px' }}>
          ...
          <Box sx={{ mb: 4 }}>
            <Stack direction="row" justifyContent="center" alignItems="center" sx={{ mb: 2.5, gap: 4 }}>
              ...
              {lessonNumber && <Typography sx={{ fontSize: '1.05rem', fontWeight: 700 }}>מס' שיעור: {lessonNumber}</Typography>}
            </Stack>
     ...
     <Button 
        variant="contained" 
        disabled={!pdfUrl}
        component="a"               // הופך לקישור אמיתי
        href={pdfUrl || ''}
        download                    // פקודה לדפדפן להוריד את הקובץ
        target="_blank"             // פותח בטאב חדש אם ההורדה לא מתחילה
